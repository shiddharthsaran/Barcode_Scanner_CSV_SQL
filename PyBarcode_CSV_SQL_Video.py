...
def execute(conn,cur,barcodedict,barcode_folder_name,barcode_marked_folder_name,output_folder_name,backup_folder_name):
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF==ord('c'):
            ...
            if(len(barcodes)<1):
                print("No new barcodes found")
                pass
            else:
                create_barcode_marked_images(conn,cur,frame,barcodes,file_name,barcodedict,barcode_folder_name,barcode_marked_folder_name,output_folder_name)
            iterate+=1
